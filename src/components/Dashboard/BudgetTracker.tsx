...
export const BudgetTracker: React.FC<BudgetTrackerProps> = ({
  ...
  const totalBudget = preferences.weeklyBudget * preferences.mealDuration;
  const budgetUsed = (currentSpend / totalBudget) * 100;
  const projectedBudgetUsed = (projectedSpend / totalBudget) * 100;
  const savings = totalBudget - projectedSpend;
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-blue-50 p-4 rounded-lg">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <span className="font-medium text-blue-800">{preferences.mealDuration}W Budget</span>
            </div>
            <p className="text-2xl font-bold text-blue-700">
              {preferences.country.currencySymbol}{totalBudget.toFixed(2)}
            </p>
          </div>
        ...
        <div>
          ...
          {projectedBudgetUsed > 100 && (
            <p className="text-sm text-red-600 mt-1">
              Over budget by {preferences.country.currencySymbol}{(projectedSpend - totalBudget).toFixed(2)}
            </p>
          )}
        </div>

        <div className="border-t pt-4">
          <h3 className="font-medium text-gray-900 mb-3">{preferences.mealDuration}-Week Breakdown</h3>
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-sm text-gray-500 pt-2 border-t">
              <span>Per Week Average</span>
              <span>{preferences.country.currencySymbol}{(projectedSpend / preferences.mealDuration).toFixed(2)}</span>
            </div>
          </div>
        </div>
      </div>
